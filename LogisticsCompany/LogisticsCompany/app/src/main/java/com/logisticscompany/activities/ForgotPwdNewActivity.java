...
public class ForgotPwdNewActivity extends AppCompatActivity {
    ...
    private void AllowAccessToAccount(final String username) {
        RootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.child("Driver_Registration").child(username).exists()) {
                    Users1 usersData = snapshot.child("Driver_Registration").child(username).getValue(Users1.class);
                    ...
                    if(!usersData.getSecurity_question_type().equalsIgnoreCase(spSQuestions.getSelectedItem().toString())){
                        Toast.makeText(ForgotPwdNewActivity.this, "Invalid Security Question", Toast.LENGTH_SHORT).show();
                    return;
                    }
                    if(!usersData.getSecurity_question_answer().equalsIgnoreCase(etSquestion.getText().toString())){
                        Toast.makeText(ForgotPwdNewActivity.this, "Invalid Security Question Answer", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    ...
                    Toast.makeText(ForgotPwdNewActivity.this, "User with this username is not exist", Toast.LENGTH_SHORT).show();
    ...
    {
        pd=new ProgressDialog(ForgotPwdNewActivity.this);
        ...
        call.enqueue(new Callback<ResponseData>() {
            ...
            public void onResponse(Call<ResponseData> call, Response<ResponseData> response) {
                ...
                if (response.body().message.equals("true")) {
                    Toast.makeText(ForgotPwdNewActivity.this, response.body().message, Toast.LENGTH_LONG).show();
                    Log.i("msg", "" + response.body().message);
                    Intent i=new Intent(ForgotPwdNewActivity.this,MainActivity.class);
                    startActivity(i);
                    finish();
                } else {
                    Toast.makeText(ForgotPwdNewActivity.this, response.body().message, Toast.LENGTH_LONG).show();
